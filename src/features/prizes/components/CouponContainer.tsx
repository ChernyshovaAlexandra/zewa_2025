...
import { DISCOUNT_EXPIRY_SHORT, PROMO_START_DATE_SHORT } from '@/shared/constants/promoDates';
...
const CouponContainer: React.FC<PrizeContainerProps> = ({ coupon }) => {
  ...
  const handleChangeToOnline = async () => {
    ...
    try {
      await apiService.changeCoupon({ barcode: coupon.barcode });
      const res = await apiService.start({ username: user?.username ?? '' });
...
Используйте промокод при заказе онлайн.`)
          ...
          <Flex justify="space-between">
            <Description style={{ marginBottom: 0 }}>
              Скидка действует до {DISCOUNT_EXPIRY_SHORT}
            </Description>
            <ToggleButton
              type="button"
              aria-label={toggledContent ? 'Скрыть описание' : 'Показать описание'}
              onClick={() => setToggled((prev) => !prev)}
            >
              <ToggleIcon rotated={toggledContent} />
            </ToggleButton>
          </Flex>
          {toggledContent && (
            <>
              ...
              <Description>
                {coupon.code ? (
                  <>
                    {applyNbsp(
                      `Акция действует в приложении «Магнит: акции и доставка» (6+) в разделе «Доставка», в котором реализуется Продукция в период с ${PROMO_START_DATE_SHORT} по ${DISCOUNT_EXPIRY_SHORT}. Скидка предоставляется в приложении на все позиции бренда Zewa при применении промокода. `,
                    )}
                    <PinkStrong>
                      {applyNbsp(
                        'Скидка не распространяется на товары с шильдиком финальная цена',
                      )}
                    </PinkStrong>
                    {applyNbsp(
                      ' и не суммируется с другими скидками и предложениями. Количество акционных товаров ограничено. В период проведения акции не гарантируется постоянное наличие полного ассортимента акционных товаров.',
                    )}
                  </>
                ) : (
                  applyNbsp(`Купон действует в магазинах «Магнит у дома», «Магнит Семейный», «Магнит Экстра», «Магнит Косметик» c ${PROMO_START_DATE_SHORT} по ${DISCOUNT_EXPIRY_SHORT}. Скидка предоставляется единоразово на кассе при предъявлении скриншота купона и действует на все позиции бренда «Zewa». Скидка распространяется на жёлтые ценники, кроме акций 1+1/2+1 и со скидкой от 2 шт и более и не суммируется с другими скидками, предоставляемыми на кассе. При единовременном действии на товар двух и более скидок, предоставляемых на кассе, покупатель получает только одну, наиболее выгодную по размеру суммы, скидку.  В одном чеке можно использовать один купон. Количество акционных товаров ограничено. В период проведение акции не гарантируется постоянное наличие полного ассортимента акционной продукции. АО «Тандер» вправе прекратить акцию досрочно.
...
const PinkStrong = styled.strong`
  color: var(--main-pink);
  font-weight: 700;
`;
