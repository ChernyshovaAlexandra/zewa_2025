...
import bridge, { EGrantedPermission } from '@vkontakte/vk-bridge';
...
const useCheckUpload = (): UseCheckUploadReturn => {
    ...
    const handleApiResponse = React.useCallback((response: any) => {
        if (response.success) {
            setMessage("Чек успешно отправлен на модерацию");
            setError(null);
        } else {
            const message = response.message ? response.message : response.response.data.message ? response.response.data.message : "Ошибка добавления чека";
            setMessage(message);
    ...
    const handleScanCheck = React.useCallback(async (scanData: CheckData) => {
        if (!vkUserData) {
    ...
    const sendDataToServer = React.useCallback((rawValue: string) => {
    ...
    const handlePhotoUpload = React.useCallback(async (photoFile: File) => {
        if (!vkUserData) {
            setError("Пользователь не авторизован");
            return;
        }
    ...
    const handleNotVKScan = React.useCallback((detectedCodes: IDetectedBarcode[]) => {
